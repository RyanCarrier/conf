---
name: herdr-worktree
description: "Spin up an isolated Herdr workspace + git worktree for one or more GitHub issues (or ad-hoc tasks) and open Claude in each with the /auto-branch command pre-typed (not submitted). Use when the user asks to start, handle, take on, work on, kick off, or spin up issue(s) in worktrees / new workspaces — e.g. 'start #146', 'handle those issues now', 'kick these off', 'spin up worktrees for the ones we just made'. One Herdr workspace per issue. Wraps the gwthcauto shell function; only works when THIS session is inside Herdr (HERDR_ENV=1)."
---
...
> There is an in-place sibling, `gwthauto` (no `c`), that does the same branch-name + worktree + typed `/auto-branch` flow **in the workspace you're already in** — no new workspace, no `-f`, quiet (no notifications). This skill is for the "spin up a fresh isolated workspace" case and uses `gwthcauto`; only reach for `gwthauto` if the user explicitly wants to take over the current workspace.
...
2. `gwthcauto` must be defined (it lives in the user's sourced profile, `~/.rcarrier_gwt.sh`). `jq`, `gh`, and `claude` must be on PATH — `gwthcauto` checks and errors clearly if not.
...
```bash
gwthcauto [-m <model>] [-f] [-g] <issue-number | "task description">
```

- `gwthcauto 146` — background; notifies when `/auto-branch` is typed and ready
- `gwthcauto -m opus 146`
- `gwthcauto --model fable make the retry backoff jittered`
- `gwthcauto -f 146` — also switch to the new workspace (default: stay put)
- `gwthcauto -g 146` — also submit `/auto-branch` (auto-launch at default effort)
...
- `-g`/`--go` when they want it to run itself — "just start it", "kick it off and let it run", "launch it", "don't wait for me", "auto-run it", "submit it". (Combine both, e.g. `gwthcauto -g -f 146`, if they ask for both.)
...
- **One workspace per issue.** For several issues, call `gwthcauto` once per issue. (Only relevant here: if two issues touch the same file, tell the user to merge them in order — that's a per-project concern, not this skill's.)
- **Default: typed-not-submitted, and background-not-focused.** `gwthcauto` leaves `/auto-branch` typed-not-submitted and the workspace unfocused, then notifies when ready. Do not send Enter into that pane and do not switch the user to it — they set effort/model and submit. Pass `-g` (submit) or `-f` (focus) **only** when the user explicitly asks for it.
...
`gwthcauto` echoes what it did (branch, worktree path, workspace id). Since this session is inside Herdr, drive and inspect the new workspace with the **`herdr` skill** (load it) or `herdr --help`:
...
- Inspect/adjust the launcher itself: the `gwthcauto` function lives in `~/.rcarrier_gwt.sh`.

Common failures:
- **Not in Herdr** (`HERDR_ENV` unset) → `gwthcauto` refuses; the user runs `gwtatauto`/`gwtauto` in a plain terminal instead.
- **Missing `jq` / `gh` / `claude`** → `gwthcauto` errors naming the missing tool.
