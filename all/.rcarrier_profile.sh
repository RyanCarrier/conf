...
# Every gwt* worktree command (gwta/gwtr/gwtat + the gwt*auto launchers) and the
# shared _gwt_branch helper live in ~/.rcarrier_gwt.sh to keep this file smaller.
# Guarded so a machine that hasn't symlinked it yet just skips them.
[ -f ~/.rcarrier_gwt.sh ] && source ~/.rcarrier_gwt.sh
